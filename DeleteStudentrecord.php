<?php
        ...
        $sql = mysqli_query($link, "SELECT `No`, `ParentId`, `PupilId`, `Absence`, `Duration` FROM `records` ");
              ...
              <table>
                ...
                while ($row = $sql->fetch_assoc()){
                  ...
                  <tr>
                    ...
                    <td>{$row['Absence']}</td>
                    <td>{$row['Duration']}</td>
                    <td><input type='checkbox' name='srecord_ids[]' value='{$row['ParentId']}'></td>
                  </tr>";
                }
                ?>
        ...
        <tr>
          <td colspan="8"><input type="submit" name="delete" value="Delete Selected Record"></td>
        </tr>
      </table>
    </form>
    <?php
...
if(isset($_POST['delete'])) {
    ...
    $srecord_ids = $_POST['srecord_ids'];
  
    // Convert array of IDs to comma-separated string
    $srecord_ids_str = implode(",", $srecord_ids);
  
    // Delete pupils from database
    $sql = "DELETE FROM Records WHERE ParentId IN ($srecord_ids_str)";
