...
input[type='checkbox'] {
  ...
  <div class="navbar">
    ...
    <form method="post">
      <table>
        <tr>
          <th width="150px">Hospital Id</th>
          <th width="250px">Pupil Id</th>
          <th width="250px">Hospital Name</th>
          <th width="250px">Doctor Name</th>
          <th width="250px">Doctor No.</th>
          <th width="250px">Allergies</th>
        ...
        while ($row = $sql->fetch_assoc()){
          ...
          <tr>
            ...
            <td>{$row['Allergies']}</td>
            <td>{$row['Address']}</td>
            <td><input type='checkbox' name='hos_ids[]' value='{$row['HospitalId']}'></td>
          </tr>";
        }
        ?>

        <tr>
          <td colspan="8"><input type="submit" name="delete" value="Delete Selected Hospital"></td>
        </tr>
      </table>
    </form>
    <?php
...
if(isset($_POST['delete'])) {
    ...
    $hos_ids = $_POST['hos_ids'];
  
    // Convert array of IDs to comma-separated string
    $hos_ids_str = implode(",", $hos_ids);
  
    // Delete pupils from database
    $sql = "DELETE FROM Hospital WHERE HospitalId IN ($hos_ids_str)";
    if(mysqli_query($link, $sql)) {
      echo "Hospital records deleted successfully.";
    } else {
      echo "Error deleting Hospital records: " . mysqli_error($link);
    }
  }
?>  
