...
$sql = mysqli_query($link, "SELECT `No`, `TeacherId`, `TeacherName`, `BankName`, `Accountno`, `Salary` FROM `PayRate`");
      ...
      <table>
        ...
        while ($row = $sql->fetch_assoc()){
          ...
          <tr>
            ...
            <td>{$row['Accountno']}</td>
            <td>{$row['Salary']}</td>
            <td><input type='checkbox' name='pay_ids[]' value='{$row['TeacherId']}'></td>
          </tr>";
        }
        ?>

        <tr>
          <td colspan="8"><input type="submit" name="delete" value="Delete Selected Pay Rate"></td>
        </tr>
      </table>
    </form>
    <?php
